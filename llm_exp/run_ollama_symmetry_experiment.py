...
PROMPT_TEMPLATES = {
    "neutral": "No rule hint. This is the original fair baseline prompt.",
    "rule_induction": "Asks the model to infer a deterministic hidden rule silently.",
    "balanced_rule_induction": "Adds that sampled query batches are approximately balanced to reduce all-negative bias.",
    "reverse_pair_probe": "Tells the model to check whether reverse ordered pairs are informative.",
    "explicit_symmetry": "Upper-bound sanity check: explicitly reveals the symmetry rule.",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="neutral",
        help="Prompt style to use. Use explicit_symmetry only as a sanity-check upper bound.",
    )
    parser.add_argument(
...
def format_examples(train_rows: pd.DataFrame, args: argparse.Namespace) -> list[str]:
    lines: list[str] = []
...
def format_queries(test_rows: pd.DataFrame, args: argparse.Namespace) -> list[str]:
    lines: list[str] = []
...
def json_output_instruction() -> list[str]:
    return [
        "Return JSON only. Do not include markdown, prose, comments, or reasoning.",
        "Use exactly this schema:",
        '{"predictions":[{"id":0,"label":0},{"id":1,"label":1}]}',
        "Return one prediction for every query id. Labels must be integers 0 or 1.",
    ]


def effective_prompt_template(args: argparse.Namespace) -> str:
    return "explicit_symmetry" if args.reveal_symmetry else args.prompt_template


def build_prompt_header(args: argparse.Namespace) -> list[str]:
    template = effective_prompt_template(args)
    common = [
        "Task: binary classification of ordered entity pairs.",
        f"The unknown binary relation is named {args.relation_name}.",
        "A label of 1 means the relation is true for the ordered pair.",
        "A label of 0 means the relation is false for the ordered pair.",
        "Entity identifiers are opaque symbols. Do not use world knowledge about names.",
    ]
    if template == "neutral":
        return common + [
            "Use only the labeled examples below to classify the queries.",
        ]
    if template == "rule_induction":
        return common + [
            "Assume labels are generated by a deterministic hidden rule.",
            "Silently infer the rule that best explains the labeled examples.",
            "Then apply that inferred rule to the query pairs.",
        ]
    if template == "balanced_rule_induction":
        return common + [
            "Assume labels are generated by a deterministic hidden rule.",
            "The sampled query batch is approximately balanced between labels 0 and 1.",
            "Do not default to predicting mostly one class.",
            "Silently infer the rule that best explains the labeled examples, then classify the queries.",
        ]
    if template == "reverse_pair_probe":
        return common + [
            "When classifying a query (a, b), explicitly check whether the labeled examples contain (b, a).",
            "If reverse ordered pairs appear informative, use that pattern consistently.",
            "If no reverse-pair evidence is available, infer the best deterministic rule from the examples.",
        ]
    if template == "explicit_symmetry":
        return common + [
            f"The relation {args.relation_name} is symmetric.",
            "Therefore, for any entities a and b, R(a,b) and R(b,a) have the same label.",
            "Use the labeled examples and this symmetry rule to classify the queries.",
        ]
...
def build_prompt(
    ...
    lines = build_prompt_header(args)
    lines.extend(["", *json_output_instruction(), "", f"Labeled examples for run {run_idx}:"])
    lines.extend(format_examples(train_rows, args))
    lines.extend(["", "Queries to classify:"])
    lines.extend(format_queries(test_rows, args))
    return "\n".join(lines)
...
def run_experiment(args: argparse.Namespace) -> None:
    if args.data_path is None:
        raise ValueError("--data-path is required unless --print-prompt-templates is used.")
    if args.output_dir is None:
        raise ValueError("--output-dir is required unless --print-prompt-templates is used.")
    ...
    aggregate_metrics.update(
        {
            ...
            or effective_prompt_template(args) == "explicit_symmetry",
            "requested_prompt_template": args.prompt_template,
            "prompt_template": effective_prompt_template(args),
            "prompt_template_description": PROMPT_TEMPLATES[effective_prompt_template(args)],
        }
    )
...
def main() -> None:
    args = parse_args()
    if args.print_prompt_templates:
        for name, description in sorted(PROMPT_TEMPLATES.items()):
